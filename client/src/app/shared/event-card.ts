...
import { availability, categoryLabel, inr, posterStyle, relativeDay, shortDate, timeOf } from '../core/format';
...
@Component({
  ...
  template: `
    @let e = event();
    <article class="event-card" role="link" tabindex="0" (keydown.enter)="open()" (click)="open()">
      ...
      <div class="card-body">
        <div class="card-date">
          {{ date() }} · {{ time() }}
          @if (soon()) {
            <span class="chip chip-soft">{{ soon() }}</span>
          }
        </div>
        <h3>{{ e.title }}</h3>
...
export class EventCard {
  ...
  protected readonly icons = { Heart, MapPin, Star };
